...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function loadReader() {
      try {
        ...
        if (mounted) {
          setIsAIReady(true);
          console.log("AI Reader ready.");
        }
      } catch (error) {
        console.error(
          "Failed to initialize AI Reader:",
          error
        );
      }
    }
  ...
  const recognizeDigit = async () => {
    ...
    if (!canvas || !isAIReady || isPredicting) {
      return;
    }
    ...
    try {
      ...
      setPrediction(result.digit.toString());

      setConfidences(result.probabilities);
    } catch (error) {
      console.error(
        ...
        <CardContent className="flex flex-col gap-4 p-4 h-full overflow-hidden justify-between">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full flex-1 overflow-hidden min-h-0">
            ...
            <div className="flex flex-col items-center justify-center border border-stone-200/80 rounded-xl p-4 bg-stone-100/40 h-full overflow-hidden">
              ...
              <div className="text-zinc-400 text-[10px] font-bold tracking-widest mb-1 uppercase">
                CORE READOUT
              </div>
            ...
            <div className="border border-stone-200/80 rounded-xl p-4 bg-stone-100/40 h-full flex flex-col justify-between overflow-hidden">
              ...
              <div className="text-zinc-400 text-[10px] font-bold tracking-widest mb-2 text-center uppercase">
                RADAR DISTRIBUTION MATRIX
              </div>
                ...
                {confidences.map(
                  (prob, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex items-center gap-2 w-full text-[11px] leading-none"
                      >

                        <span
                          className={`w-3 font-mono font-bold ${
                            isMatch
                              ? "text-zinc-900"
                              : "text-stone-400"
                          }`}
